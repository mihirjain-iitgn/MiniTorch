...
def backpropagate(variable, deriv):
    ...
    # The code below traverses the computation Graph in a BFS

    Q = Queue()
    Q.put((variable,deriv))
    while not Q.empty():
        cur_node = Q.get()
        if cur_node[0].is_leaf():
            # Saving derivatives of nodes with furthur links.
            cur_node[0].accumulate_derivative(cur_node[1])
        else:
            # Putting all nodes that were inputs to the creatin of this node in the Queue.
            prev_nodes_deriv = cur_node[0].history.backprop_step(cur_node[1])
            for node_deriv in prev_nodes_deriv:
                Q.put(node_deriv)
