...
def broadcast_index(big_index, big_shape, shape, out_index):
    ...
    for idx, s in enumerate(shape):
        if s == 1:
            ...
            out_index[idx] = big_index[idx + right_shift]
...
def shape_broadcast(shape1, shape2):
    ...
    if n1 != n2:
        ...
        left_pad = (n1-n2)*[1]
        shape2 = tuple(left_pad + list(shape2))
    n_shape = [0 for _ in range(n1)]
    for i in range(n1):
        if shape1[i] == shape2[i]:
            n_shape[i] = shape1[i]
        elif shape1[i] == 1 or shape2[i] == 1:
            n_shape[i] = max(shape1[i], shape2[i])
        else:
...
class TensorData:
    ...
    def permute(self, *order):
        ...
        dims = len(self.shape)
        n_shape = [0 for _ in range(dims)]
        n_stride = [0 for _ in range(dims)]
        for idx, n_idx in enumerate(list(order)):
            n_shape[idx] = self.shape[n_idx]
            n_stride[idx] = self.strides[n_idx]
        n_shape = tuple(n_shape)
        n_stride = tuple(n_stride)
        return TensorData(self._storage, n_shape, n_stride)
