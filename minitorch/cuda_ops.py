...
def _sum_practice(out, a, size):
    ...
    if size > pos:
        ...
        # out[pos // BLOCK_DIM] = out[pos // BLOCK_DIM] + a[pos]
...
@cuda.jit()
def tensor_matrix_multiply(out, out_shape, out_strides, out_size, a_storage, a_shape, a_strides, b_storage, b_shape, b_strides):
    """
    ...
    pos = cuda.blockDim.x * cuda.blockIdx.x + cuda.threadIdx.x
    if out_size > pos:
        out_index = cuda.local.array(MAX_DIMS,numba.int32)
        a_index = cuda.local.array(MAX_DIMS,numba.int32)
        b_index = cuda.local.array(MAX_DIMS,numba.int32)
